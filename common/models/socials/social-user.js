...
module.exports = function (UserInfo) {
  ...
  /**
     ...
     *
     */
  UserInfo.prototype.createAccessToken = function (ttl, options, cb) {
   ...
   */
  UserInfo.normalizeCredentials = function (credentials, realmRequired, realmDelimiter) {
    var query = {};
   ...
   *
   * ```js
   *    UserInfo.login({username: 'foo', password: 'bar'}, function (err, token) {
   ...
   * @param {string} credentials.username username/password or email/password
   * @param {string} credentials.password username/password or email/password
  ...
  UserInfo.login = function (username, password, include, fn) {
    var self = this;
    ...
    self.findOne({ where: query }, function (err, user) {
      ...
      function tokenHandler(err, token) {
        ...
        fn(err, response.response_code, response.message, response.data);
      }
   ...
   *
   * ```js
   *    UserInfo.logout('asd0a9f8dsj9s0s3223mk', function (err) {
  ...
  *    });
  ...
  UserInfo.logout = function (tokenId, fn) {
    fn = fn || utils.createPromiseCallback();
  ...
  UserInfo.observe('before delete', function (ctx, next) {
  ...
  UserInfo.prototype.hasPassword = function (plain, fn) {
    fn = fn || utils.createPromiseCallback();
   ...
   * @promise
   */
  UserInfo.changePassword = function (userId, oldPassword, newPassword, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
   ...
   * @promise
   */
  UserInfo.prototype.changePassword = function (oldPassword, newPassword, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
   ...
   * @promise
   */
  UserInfo.setPassword = function (userId, newPassword, options, cb) {
   ...
   * @promise
   */
  UserInfo.prototype.setPassword = function (newPassword, options, cb) {
    assert(!!newPassword, 'newPassword is a required argument');
  ...
  UserInfo.getVerifyOptions = function () {
    const verifyOptions = {
      type: 'email',
   ...
   * ```
   *
   * NOTE: the UserInfo.getVerifyOptions() method can also be used to ease the
   * building of identity verification options.
   *
   * ```js
   * var verifyOptions = MyUserInfo.getVerifyOptions();
   * user.verify(verifyOptions);
   * ```
   *
  ...
  UserInfo.prototype.verify = function (verifyOptions, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
   ...
   */
  UserInfo.generateVerificationToken = function (user, options, cb) {
   ...
   * @promise
   */
  UserInfo.confirm = function (uid, token, redirect, fn) {
    fn = fn || utils.createPromiseCallback();
  ...
  UserInfo.resetPassword = function (options, cb) {
    cb = cb || utils.createPromiseCallback();
    var UserModel = this;
  ...
  UserInfo._invalidateAccessTokensOfUsers = function (userIds, options, cb) {
    ...
    var isRelationPolymorphic = relatedUser && relatedUserInfo.polymorphic &&
      !relatedUser.modelTo;
  ...
  UserInfo.setup = function () {
    // We need to call the base class's setup method
    UserInfo.base.setup.call(this);
    var UserModel = this;
    ...
    UserModel.remoteMethod(
      'login',
      {
        ...
        accepts: [
          { arg: 'login', type: 'string', required: true },
  ...
  UserInfo.observe('access', function normalizeEmailCase(ctx, next) {
  ...
  UserInfo.observe('before save', function rejectInsecurePasswordChange(ctx, next) {
  ...
  UserInfo.observe('before save', function prepareForTokenInvalidation(ctx, next) {
  ...
  UserInfo.observe('after save', function invalidateOtherTokens(ctx, next) {
  ...
  UserInfo.createUserInfo = function (user) {
    return new Promise(function (resolve, reject) {
      var pass = UserInfo.hashPassword(user.password);
      user.password = pass;
      // user.album_name = `@${user.username}_${user.id}`
      UserInfo.create(user)
        .then(data => {
          return UserInfo.updateAll({
            id: data.id.toString()
          }, {
              albumname: `@${data.username}_${data.id.toString()}`
            })
        })
        .then(data => {
        resolve(data)
...
function joinUrlPath(args) {
   ...
   */
  UserInfo.getUserInfoByListUser = function (list_user, array_field) {
    if (!array_field)
  ...
  /**
       ...
       * @param {[number]} - list_user [user_id:15,'user_id':16]
       */
  UserInfo.findUser = function (user_id, list_user) {
    let userSelection = {}
  ...
  // UserInfo.createSocialUser = function (user, username, email, cb) {
  ...
  */
  UserInfo.beforeRemote('updateSocialUser', function (ctx, any, next) {
    // đẩy nội dung media vào req
    multerArray(ctx.req, ctx.res, next)
  })
  UserInfo.updateSocialUser = function (req, res, cb) {
  ...
  UserInfo.remoteMethod('updateSocialUser', {
   ...
   * @using by comment
   */
  UserInfo.getByUser_id = function (user_id, array_field) {
    if (!array_field)
      array_field = ['username', 'displayName', 'avatar']
   ...
   * @using by comment
   */
  UserInfo.getByUser_name = function (username, array_field) {
